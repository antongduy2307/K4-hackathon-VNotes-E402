"""Thin HTTP layer for the two export buttons in the UI ("Lưu vào Anki" /
"Lưu vào Notion"). Deliberately NOT LLM-invoked — export is a deterministic
action the user clicks, not something worth routing through the agent's tool
loop. The UI is expected to call note_capture (via the agent) first to get
the filtered notes, then POST that same list here.

    ...
    for case in cases:
        for call in case.get("expect", {}).get("tool_calls", []):
            ...
            if name not in declared:
                ...
                invalid.append(f"{case.get('id', '<missing id>')}: {name!r} has no implementation in tools/")
    if invalid:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run VLearn Tutor agent live evals.")
    parser.add_argument("--phase", choices=["B"], default="B")
    parser.add_argument("--suite", default="base", help="Run label saved to JSON; does not filter --eval-cases.")
    ...
    for case in cases:
        ...
        agent = VLearnAgent(provider, system_prompt=system_prompt, tools=openai_tools, model=args.model, session=session)
        try:
